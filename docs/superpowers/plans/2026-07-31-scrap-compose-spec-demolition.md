...
- **`README.md` and `docs/REFERENCE.md` are DERIVED, and a test gates them.** They list the skills and their counts, so any task that adds or removes a skill makes them stale and turns `engine/tests/test_docsfile.py` red. Regenerate with `abu build-docs` (run from the repo root; `python3 -m agenticstory.cli build-docs` if the wrapper is not on PATH) and fold the regenerated docs into the SAME commit as the change that invalidated them. Never hand-edit either file. Discovered in Task 1, where the suite went red at 564 tests until the docs were rebuilt.
