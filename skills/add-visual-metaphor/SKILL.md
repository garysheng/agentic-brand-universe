...
6. **Validate + commit.** `agenticstory validate <universe>` stays green (an `unlocked` visual-metaphor still validates: that is correct, not an error). Commit the entity + reference dir + prompts.md. Report `lock_level: stub` and that the entity stays refused by `assert-story`/`assert-spread` until `shoot-references` fills every plate and flips `status` to `"locked"`.
