...
| **show the operator every shot and record that they saw it** | `shoot-references/scripts/shot_board.py board --universe <u> --entity <id> --json <pngs>` then `... tap <png> --verdict keep`. A tap on an AskUserQuestion card IS what seen means (v0.50), and `lock-shot` REFUSES a shot with no approving verdict. Do not rewrite the questions it hands back: the off-list answer is already in the question text, because a preview costs the visible `Other` row. |
